...
class ShopifyConfig(models.Model):
    ...
    last_sync = fields.Datetime(string="Last Successful Sync", copy=False, readonly=True)
    ...
    def _get_or_create_category(self, product_type):
        ...
        if not category:
            category = self.env["product.category"].create({"name": product_type})
        return category
    ...
    def _sync_single_variant(self, template, variant):
        ProductVariant = self.env["product.variant"]
    ...
    def _get_or_create_customer(self, shopify_order):
        ...
        partner = Partner.search([("email", "=", email), ("type", "=", "contact")], limit=1)
        if partner:
            return partner
